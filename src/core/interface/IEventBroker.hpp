...
class IEventBroker
{
   public:
    ...
    {
        subscribeImpl(std::type_index(typeid(Event)), [cb = std::move(callback)](const void* evt) {
   ...
   protected:
    ...
    virtual void subscribeImpl(std::type_index type, std::function<void(const void*)> callback);
