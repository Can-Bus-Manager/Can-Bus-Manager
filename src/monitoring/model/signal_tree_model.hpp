...
public slots:
    void onFrameReceived(const Core::CanFrame& frame);

    signals:
        void signalChecked(const Core::DecodedSignal& signal);
    void signalUnchecked(const Core::DecodedSignal& signal);

private:
    struct SignalNode {
        Core::DecodedSignal signal;
